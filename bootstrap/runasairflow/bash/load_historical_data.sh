...
if [ ${rc} -eq 0 ]; then
  ...
  sed "s/START_DATE_AS_PY_CODE/${START_DATE_AS_PY_CODE}/g" /opt/orchestrator/bootstrap/runasairflow/python/dags/ga_chu_prediction_pipeline.py.template > /home/airflow/airflow/dags/ga_chu_prediction_pipeline.py
  start_airflow.sh
